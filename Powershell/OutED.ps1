...
Param (
	...
	#[string[]]$SearchOU = ("OU=YOUR-OU"),              #CHANGEME AND REMOVE COMMENT
    #[String[]]$LocationDefault = ("DEFAULT-LOCATION"), #CHANGEME AND REMOVE COMMENT
    
    [ValidateScript({Test-Path $_ -PathType Container})]
	#[string]$OutputPath = "\\SERVER\SHARE",            #CHANGEME AND REMOVE COMMENT
    
    [ValidateScript({Test-Path $_})]
    [string]$HTMLPath,
    
    [ValidatePattern("Name|Picture|Department|Internal|Cell|Ext|Email|Title")]
    [string]$Fields = "Name,Picture,Department,Internal,Cell,Ext,Email,Title",
    
    [ValidatePattern("Name|Picture|Department|Internal|Cell|Ext|Email|Title")]
    [string]$SortBy = "Name",
    
    [ValidatePattern("Department|Location")]
    [string]$ButtonBy = "Department",
...
$Regex = "Name|Picture|Department|Internal|Cell|Ext|Email|Title"
...
{   Write-Error "ButtonBy must be either Department, or Location.  Set to: $ButtonBy"
    Exit
}
Else
{   $FilterColumn = 0
...
thead th, tfoot th { background:#23185a; 
 color:#FFFFFF; 
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-150277535-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-150277535-1');
</script>
"@ #End EndHeaderHTML
...
Function Set-Grid {
    ...
    Process {
        ...
        {   
            ...
            $Line = $Line.Replace("Ext","Telephone")
            $Line = $Line.Replace("Cell","Mobile")
            $Line = $Line.Replace("physicalDeliveryOfficeName","Location")
            $Line = $Line.Replace("[","&#91;")
            $Line = $Line.Replace("]","&#93;")
            $HTMLOutput += $Line
        } #End ForEach
...
$PropertiesToLoad = "SamAccountName,useraccountcontrol,distinguishedname,GivenName,sn,Title,cn,description,department,pager,physicaldeliveryofficename,manager,TelephoneNumber,Mobile,facsimiletelephonenumber,mail,thumbnailphoto,url"
...
{	#Filter out users who don't fall within the search parameters
    ...
    #Filter out users with the word 'Exclude' in the fax field
	If ($($User.Properties.facsimiletelephonenumber))
	{	If ($($User.Properties.facsimiletelephonenumber).ToUpper() -eq "EXCLUDE")
		{	Continue
		}
    }
